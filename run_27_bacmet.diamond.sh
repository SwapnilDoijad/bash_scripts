...
###############################################################################
	...
	#if [ ! -f /media/swapnil/share/databases/bacmet/BacMet_EXP.704.Header_only_Bac-Id.dmnd ] && [ ! -f /media/swapnil/share/databases/bacmet/BacMet_PRE.40556.Header_only_Gi.dmnd ] ; then
    #diamond makedb --in /media/swapnil/share/databases/bacmet/BacMet_PRE.40556.Header_only_Gi.fasta -d /media/swapnil/share/databases/bacmet/BacMet_PRE.40556.Header_only_Gi
	#diamond makedb --in /media/swapnil/share/databases/bacmet/BacMet_EXP.704.Header_only_Bac-Id.fasta -d /media/swapnil/share/databases/bacmet/BacMet_EXP.704.Header_only_Bac-Id
    #fi
...
for F1 in $(cat $list); do
	...
	if [ -f results/27_BacMet"$s"/tmp_EXP/$F1.BacMet_EXP.tmp ]; then
		...
		(diamond blastp -b 8 -d /media/swapnil/share/databases/bacmet/BacMet_EXP.704.Header_only_Bac-Id.dmnd \
		-q results/08_annotation/raw_files/$F1/$F1.faa --id 70 --query-cover 85 --max-target-seqs 1 \
		-o results/27_BacMet"$s"/tmp_EXP/$F1.BacMet_EXP.tmp \
		-f 6 sseqid qseqid sstart send qstart qend slen qlen evalue bitscore length mismatch gaps pident nident )> /dev/null 2>&1

		(diamond blastp -b 8 -d /media/swapnil/share/databases/bacmet/BacMet_PRE.40556.Header_only_Gi.dmnd \
		-q results/08_annotation/raw_files/$F1/$F1.faa --id 70 --query-cover 85 --max-target-seqs 1 \
		-o results/27_BacMet"$s"/tmp_PRE/$F1.BacMet_PRE.tmp \
		-f 6 sseqid qseqid sstart send qstart qend slen qlen evalue bitscore length mismatch gaps pident nident )> /dev/null 2>&1
		...
		awk -F'\t' -f /home/swapnil/pipeline/tools/vlookup-BacMet.awk \
		/media/swapnil/share/databases/bacmet/BacMet_both.mapping.tab \
		results/27_BacMet"$s"/tmp/$F1.BacMet.list.tmp > results/27_BacMet"$s"/tmp/$F1.BacMet.list.metadata.tmp
		...
		ex -sc '1i|sseqid qseqid sstart send qstart qend slen qlen evalue bitscore length mismatch gaps pident nident gene protein group originated-from gene-name location organism compound description' -cx results/27_BacMet"$s"/$F1.BacMet.tab
		sed -i '1s/ /\t/g' results/27_BacMet"$s"/$F1.BacMet.tab

		#------------------------------------------------------------------------------

		# echo "finished... BacMet for $F1"
